...
test.describe('Search Functionality - Car Park Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test('@smoke @search TC-03: Search for "car park" returns relevant listings', async ({ page }) => {
    ...
    test('@regression @search TC-04: All "car park" search results contain relevant keywords', async ({ page }) => {
      await homePage.search('car park');
    ...
    test('@smoke @search TC-05: "car park" search displays listings with correct information', async ({ page }) => {
      await homePage.search('car park');
    ...
    test('@smoke @search TC-04: Search URL contains correct search parameters', async ({ page }) => {
    await homePage.search('car park');
    ...
    test('@regression @search TC-05: Search with numeric values returns results', async ({ page }) => {
  ...
  test.describe('Search Functionality - Negative Scenarios', () => {
    ...
    test('@regression @search TC-09: Search with empty input shows appropriate behavior', async ({ page }) => {
    ...
    test('@regression @search TC-10: Search with special characters only returns no results gracefully', async ({ page }) => {
    ...
    test('@regression @search TC-11: Search with extremely long string handles gracefully', async ({ page }) => {
    ...
    test('@regression @search TC-12: Search with non-existent terms shows no results message', async ({ page }) => {
  ...
  test.describe('Search Functionality - Edge Cases', () => {
    ...
    test('@regression @search TC-16: Search with mixed case returns case-insensitive results', async ({ page }) => {
    ...
    test('@regression @search TC-17: Search with accents works correctly', async ({ page }) => {
  ...
  test('@performance @search TC-20: Search results load within acceptable time', async ({ page }) => {
  ...
  test.describe('Search Filters - House Search', () => {
    ...
    test('@regression @filters TC-20: Category dropdown filters results', async ({ page }) => {
      await homePage.search('house');
      ...
      test('@regression @filters TC-21: Location dropdown filters results', async ({ page }) => {
        await homePage.search('house');
   ...
   await page.getByRole('button', {
    ...
    await expect(page.locator('button:has-text("North Shore")')).toBeVisible({ timeout: 3000 });
      ...
      test('@regression @search TC-30: Clicking X clears the search bar', async ({ page }) => {
  ...
  test('@regression @filters TC-22: Condition dropdown filters results', async ({ page }) => {
    await homePage.search('house');
  ...
  test('@smoke @filters TC-23: Shipping dropdown displays results correctly', async ({ page }) => {
    ...
    expect(
      ...
      test('@regression @filters TC-24: Sort dropdown changes result ordering', async ({ page }) => {
        await homePage.search('house');
...
).toHaveValue(/Sort: Latest listings/i);

        // Store first result after sorting
        const firstResultAfter = await searchResultsPage.getFirstListingText();

        // Verify order changed
        expect(firstResultBefore).not.toEqual(firstResultAfter);
      });

      test('@regression @filters TC-25: Multiple filters work together', async ({ page }) => {
        await homePage.search('house');
